...
export class CreateCommunityComponent {
  ...
  validateStep(step: number): boolean {
    ...
    if (step === 1) {
      if (!this.community.name.trim()) {
        this.errors.name = 'Le nom de la communauté est obligatoire.';
      }
      ...
      if (!this.community.category) {
        this.errors.category = 'Sélectionnez une catégorie.';
      }

      if (!this.community.description.trim()) {
        this.errors.description = 'Ajoutez une courte description.';
      }
    ...
    if (step === 2) {
      if (!this.community.longDescription.trim()) {
        this.errors.longDescription = 'Présentez votre communauté plus en détail.';
      }
    ...
    if (step === 3) {
      if (!this.community.rules.trim()) {
        this.errors.rules = 'Ajoutez quelques règles pour votre communauté.';
      }
  ...
  onCoverSelected(event: Event): void {
    ...
    this.community.cover = URL.createObjectURL(file);
    input.value = '';
    this.cdr.markForCheck();
  }
  ...
  createCommunity(): void {
    ...
    /* =========================
       LIMIT STANDARD
    ========================== */
    const myCommunities = this.communityService.getMyCommunities();
    ...
    const newCommunity = this.communityService.createCommunity({
      ...
      longDescription: this.community.longDescription.trim(),
      icon: this.community.icon,
      cover: this.community.cover,
      isPublic: this.community.isPublic,
      allowMembers: this.community.allowMembers,
      allowComments: this.community.allowComments,
      allowLives: this.community.allowLives,
      liveCapacity: this.community.liveCapacity,
      liveDuration: this.community.liveDuration,
  ...
  get previewName(): string {
    return this.community.name.trim() || 'Nom de votre communauté';
  }

  get previewCategory(): string {
    return this.community.category || 'Catégorie';
  }

  get previewDescription(): string {
    return this.community.description.trim() || 'Votre description apparaîtra ici.';
  }
  ...
  get previewCover(): string {
    ...
    return `linear-gradient(135deg, #1769e0, #0d47a1)`;
  }
